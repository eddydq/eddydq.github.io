(function(root, factory) {
    ...
    function encodeParams(blockId, params = {}) {
        ...
        case 'source.lis3dh':
            return [
                ...u16(params.sample_rate_hz ?? params.sampleRateHz ?? 100),
                clampByte(params.resolution ?? 12)
            ];
        case 'mpu6050_source':
        case 'source.mpu6050':
            return [
                ...u16(params.sample_rate_hz ?? params.sampleRateHz ?? 100),
                clampByte(params.resolution ?? 16)
            ];
        case 'polar_source':
        case 'source.polar':
            return [clampByte(params.axis_mask ?? params.axisMask ?? 7)];
        case 'select_axis':
